...
function createScene() {
	...
	document.body.appendChild(generateScore());
	document.body.appendChild(generateInfoText());
	//stats = new Stats();
	...
	createTreesPool();
	scene.add(generateWorld());
	scene.add(generateHero());
	scene.add(generateLight());
	scene.add(generateSun());
	scene.add(generateExplosion());
...
function generateExplosion() {
	var pMaterial = new THREE.ParticleBasicMaterial({ color: 0xfffafa, size: 0.2 });
...
function generateLight() {
	return new THREE.HemisphereLight(0xfffafa, 0x000000, .9);
}
